...
import type { TaxInvoice, Project } from '../types';
...
export const TaxInvoices: React.FC = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const txs = await api.getTaxInvoices();
  ...
  // Metrics
  const totalTaxAmount = taxInvoices.reduce((sum, tx) => sum + tx.amount, 0);
  const totalGstCollected = totalTaxAmount * 0.18; // 18% GST estimate
  const issuedCount = taxInvoices.filter(tx => tx.status === 'Issued').length;
  const pendingCount = taxInvoices.filter(tx => tx.status === 'Pending').length;
  ...
  const filteredTxs = taxInvoices.filter(tx => {
    ...
    const matchesProject = selectedProject === 'All' || tx.projectNo === selectedProject;
  ...
  if (loading) {
    return <div className="text-secondary font-headline p-8">Loading Tax Invoices...</div>;
  }
    ...
    <div className="space-y-stack-lg max-w-[1400px] mx-auto w-full">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="font-sans text-sm text-secondary mt-1">Official GST compliances records and linked client invoice tax allocations.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-surface-container-lowest border border-outline-variant rounded-md p-5 flex flex-col shadow-sm">
          <span className="text-secondary font-headline text-xs font-semibold uppercase tracking-wider mb-2">Total Tax Invoices</span>
          <span className="font-headline text-2xl font-bold text-on-surface">{taxInvoices.length} Sheets</span>
        </div>
        <div className="bg-surface-container-lowest border border-outline-variant rounded-md p-5 flex flex-col shadow-sm">
          <span className="text-secondary font-headline text-xs font-semibold uppercase tracking-wider mb-2">Total Tax Value</span>
          <span className="font-headline text-2xl font-bold text-on-surface">{formatINR(totalTaxAmount)}</span>
        </div>
        <div className="bg-surface-container-lowest border border-outline-variant rounded-md p-5 flex flex-col shadow-sm">
          <span className="text-secondary font-headline text-xs font-semibold uppercase tracking-wider mb-2">Est. GST Collected</span>
          <span className="font-headline text-2xl font-bold text-primary">{formatINR(totalGstCollected)}</span>
        </div>
        <div className="bg-surface-container-lowest border border-outline-variant rounded-md p-5 flex flex-col shadow-sm">
          <span className="text-secondary font-headline text-xs font-semibold uppercase tracking-wider mb-2">Filing Status</span>
          <span className="font-headline text-2xl font-bold text-secondary">{issuedCount} Filed &bull; {pendingCount} Draft</span>
        </div>
      </div>
      ...
      <div className="bg-surface-container-lowest border border-outline-variant rounded-md p-4 flex flex-wrap items-center gap-4 shadow-sm">
        ...
        <div className="flex items-center gap-2 bg-surface px-3 py-1 rounded-lg border border-outline-variant font-sans">
          ...
          >
            ...
            <option value="Issued">Issued / Filed</option>
        ...
        <div className="ml-auto flex items-center gap-2">
          <button 
            onClick={() => alert('Tax invoices exported.')}
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-surface-container-low border-b border-outline-variant">
                <th className="px-6 py-4 font-headline text-xs font-bold text-secondary uppercase tracking-wider">Tax Invoice #</th>
            ...
            <tbody className="divide-y divide-outline-variant">
              {paginatedTxs.length === 0 ? (
                <tr>
                  <td colSpan={8} className="px-6 py-8 text-center text-secondary font-headline">No tax invoices found.</td>
                </tr>
              ) : (
                paginatedTxs.map((tx) => (
                  <tr key={tx.id} className="hover:bg-surface transition-colors">
                    <td className="px-6 py-4 font-headline text-sm font-bold text-primary">{tx.id}</td>
                    <td className="px-6 py-4">
                      <div className="flex flex-col font-sans">
                        <span className="text-sm font-medium text-on-surface">{tx.projectName}</span>
                        <span className="text-xs text-secondary mt-0.5">{tx.projectNo}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 font-sans text-sm text-secondary">{tx.customer}</td>
                    <td className="px-6 py-4 text-right font-headline text-base font-bold text-on-surface">{formatINR(tx.amount, false)}</td>
                    <td className="px-6 py-4 font-sans text-sm text-primary underline">{tx.linkedInvoiceNo}</td>
                    <td className="px-6 py-4">
